...
public class ExactMatchWinningStrategy implements WinningStrategy {
    ...
    public int calculateNewPosition(int currentPosition, int diceRoll, int boardSize) {
        ...
        if (newPosition > boardSize) {
            // In exact match strategy, if the roll would overshoot, player stays at current position
            System.out.println("Exact match required! Roll would overshoot the board. Staying at position " + currentPosition);
